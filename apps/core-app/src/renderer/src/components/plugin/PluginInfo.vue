<template>
  <div class="PluginInfo-Container">
    ...
    <FormTemplate content-style="width: calc(100% - 5rem);height: calc(100% - 15rem)">
      <template #header>
        <div grid gap-4 grid-cols-8 items-center>
          ...
          <FlatButton v-if="plugin.dev?.enable" h-5 w-40 right-6 @click="handleExplorer">
      ...
      <BlockTemplate title="Overview">
        <LineTemplate title="mode">
          <span v-if="plugin.dev?.enable" text-sm class="plugin-dev">Dev</span>
          <span v-else>-</span>
        </LineTemplate>
      ...
      <BlockTemplate v-if="platforms" title="Environment">
        ...
        >
          ...
          <el-tag v-for="(tag, index) in platform!.os" :key="index" size="small" type="primary">{{
            tag
          }}</el-tag>
          <el-tag v-for="(tag, index) in platform!.arch" :key="index" size="small" type="info">{{
            tag
          }}</el-tag>
        </LineTemplate>
      </BlockTemplate>
      ...
      <BlockTemplate v-if="plugin.readme" :style="`padding-right: 1.25rem`" title="Readme">
