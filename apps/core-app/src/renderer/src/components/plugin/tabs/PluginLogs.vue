  ...
  <div class="PluginLogs w-full space-y-6">
    ...
    <div class="PluginLogs-Controls flex justify-between items-center gap-4">
      ...
      <div class="PluginLogs-Actions flex gap-2">
        <button
          class="PluginLogs-ActionButton bg-white/5 border border-white/10 rounded-lg w-9 h-9 flex items-center justify-center text-white/60"
          @click="clearLogs"
          title="Clear logs"
        >
    ...
    <!-- Logs Container -->
    <div
      class="PluginLogs-Container bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden"
    >
      <div v-if="filteredLogs.length" class="PluginLogs-List max-h-96 overflow-y-auto">
        ...
        >
          <span class="PluginLogs-Time text-white/50 min-w-20">{{
    ...
    <div class="PluginLogs-Stats grid grid-cols-2 md:grid-cols-4 gap-4">
      <div
        class="PluginLogs-StatCard bg-red-500/10 border border-red-400/20 rounded-xl p-4 text-center"
      >
        <div class="text-xl font-bold text-red-300">{{ getLogCount('error') }}</div>
        <div class="text-xs text-red-200/60">Errors</div>
      </div>
      <div
        class="PluginLogs-StatCard bg-yellow-500/10 border border-yellow-400/20 rounded-xl p-4 text-center"
      >
        <div class="text-xl font-bold text-yellow-300">{{ getLogCount('warn') }}</div>
        <div class="text-xs text-yellow-200/60">Warnings</div>
      </div>
      <div
        class="PluginLogs-StatCard bg-blue-500/10 border border-blue-400/20 rounded-xl p-4 text-center"
      >
        <div class="text-xl font-bold text-blue-300">{{ getLogCount('info') }}</div>
        <div class="text-xs text-blue-200/60">Info</div>
      </div>
      <div
        class="PluginLogs-StatCard bg-gray-500/10 border border-gray-400/20 rounded-xl p-4 text-center"
      >
