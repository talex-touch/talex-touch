...
export const VectorIndexLength = 256 * 256 * (4 + 4);
export const SegmentIndexSize = 14;
export const IP_REGEX = /^((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.){3}(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])$/;

export const getStartEndPtr = Symbol('#getStartEndPtr');
export const getBuffer = Symbol('#getBuffer');
export const openFilePromise = Symbol('#openFilePromise');
...
export class Searcher {
  ...
  async search (ip) {
    ...
    if (!isValidIp(ip)) {
      return { region: "Local", ioCount: 1, took: 0 };
...
export const _checkFile = dbPath => {
  try {
...
export const newWithFileOnly = dbPath => {
  _checkFile(dbPath);

  return new Searcher(dbPath, null, null);
};

export const newWithVectorIndex = (dbPath, vectorIndex) => {
  _checkFile(dbPath);
...
export const newWithBuffer = buffer => {
...
const loadContentFromFile = dbPath => {
  console.log("IpStorage", "Load ip data from: " + chalk.black.bgWhite(` ${dbPath} `))
...
// import Ip2Region from './ip2region.xdb'

const buffer = loadContentFromFile(path.resolve('.', 'ip2region.xdb'));
