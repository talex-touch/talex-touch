    ...
    <t-group-block :name="`${$t('settings.application.list-settings.specifications.name')} (Touch)`" icon="apps">
      ...
      <t-block-line :title="$t('settings.application.list-settings.specifications.time')">
        <template #description>
          ...
          <span class="tag" style="color: var(--el-color-warning)" v-else-if="startCosts < 5">
            启动较慢
          </span>
          <span class="tag" style="color: var(--el-color-error)" v-else-if="startCosts < 10">
            启动缓慢
          </span>
          <span class="tag" style="color: var(--el-color-error); font-weight: 600" v-else-if="startCosts < 30">
            启动滞后
          </span>
