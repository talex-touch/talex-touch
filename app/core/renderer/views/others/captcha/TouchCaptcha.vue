<template>
  <div :class="{ success: options.success }" class="TouchCaptcha-Container fake-background">
    <ProgressBar :message="options.message" :success="options.success" :error="options.error" :loading="options.loading" />

    <h2 class="TouchCaptcha-Title">
      {{ $t('base.touchCaptcha') }}
    </h2>
    <span>旋转滑块以匹配图像</span>
    <div ref="content" :class="{ loading: options.loading, error: options.error, success: options.success }" class="TouchCaptcha-Content">
    ...
    <div :class="{ loading: options.loading || options.error || options.success }" class="TouchCaptcha-Slider">
      <el-slider @change="validateCaptcha" :min="90" :max="270" :show-tooltip="false" v-model="options.img.style.transform.rotate">
...
async function validateCaptcha() {
  ...
  const res = await post('/file/captcha/valid', {
    captcha: 360 - angle,
  }, {
    headers: {
      captcha: id
...
async function gen() {
  ...
  options.img.style.transform.scale = .8
...
.TouchCaptcha-Content {
  ...
  &.loading {
    ...
    animation: breathing .75s infinite;
  }
  img {
    position: absolute;
